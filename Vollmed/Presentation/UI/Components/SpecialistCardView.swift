...
struct SpecialistCardView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            if !isAppointmentView {
                ...
                HStack {
                    NavigationLink {
                        AgendamentoView(isRescheduleView: true, specialist: specialist, appointment: appointment)
                    } label: {
    ...
    func updateDoctorImage() async {
        do {
            guard let receivedImage = try await WebService.shared.asyncImage(from: specialist.imageUrl) else {
                return
            }
            image = receivedImage
        } catch {
